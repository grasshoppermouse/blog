...
Most social scientists test a substantive hypothesis, which we will call $H_1$, by instead testing a null hypothesis, $H_0$, which is usually something like "the mean of X equals 0" or "the correlation of X and Y equals 0." If the probability of the data[^1] under $H_0$ is low (e.g., $p < 0.05$), the researcher rejects $H_0$, which is taken as evidence in favor of $H_1$.

This approach, termed [null hypothesis significance testing](https://en.wikipedia.org/wiki/Statistical_hypothesis_testing) (NHST), has been subjected to scathing criticism. In the social sciences, for example, we know $H_0$ is false before we collect a single data point --- the mean of X is never exactly 0, the correlation of two variables is never exactly 0, and so forth. Hence, the rejection of such a null, which [Cohen (1994)](http://dx.doi.org/10.1037/0003-066X.49.12.997) termed a "nil" hypothesis, just depends on sample size:
...
In the natural sciences, such as physics and chemistry, theories typically predict numerical values for various parameters. Sometimes the predicted value might be 0. Under Einstein's theory of [special relativity](https://en.wikipedia.org/wiki/Special_relativity), for instance, the speed of light in the direction of the earth's motion [will not differ](https://en.wikipedia.org/wiki/Michelson–Morley_experiment) from the speed of light perpendicular to the earth's motion. Other times, the predicted values are different from 0. Einstein's theory of [general relativity](), for example, predicts that the sun will bend star light by a [specific (non-zero) amount](https://en.wikipedia.org/wiki/Tests_of_general_relativity). In either case, the predicted value is a substantive $H_0$ (not a nil) that can be meaningfully tested with NHST.

Unlike rejecting a nil, which social scientists usually take as evidence supporting their theory, rejecting a substantive $H_0$ *rejects* the theory! If the difference of the speed of light in one direction vs. another is greater than 0 $(p < 3 \times 10^{-7})$, then special relativity is wrong![^4] If the sun bends light more or less than the predicted amount $(p < 3 \times 10^{-7})$ then general relativity is wrong![^2] In the natural sciences, NHST is often a powerful tool to *challenge* theories, not support them (see @fig-NHST).

```{r}
#| label: fig-NHST
#| fig-height: 6
#| fig-cap: "Testing nils vs. substantive nulls. Black vertical line: null. Red vertical line: mean of data. **A**: Rejecting the nil that the mean = 0 is taken as support for a theory predicting that the mean has some value > 0. **B**: Rejecting the substantive null that the mean = 0 rejects the theory. **C**: Failing to reject the substantive null that the mean = 3 fails to reject the theory. All data simulated."
...
arrow_y <- 25
y_max <- 30
arrw <- arrow(length = unit(.125, 'inches'))
...
p1 <-
  ...
  geom_vline(xintercept = mean_x, colour = 'red') +
  scale_x_continuous(limits = c(-1, 5)) +
  scale_y_continuous(limits = c(0, y_max)) +
  annotate('segment', x = 0, y = arrow_y, xend = 2.9, yend = arrow_y, arrow = arrw) +
  annotate('text', x = 3, y = 19, label = 'Theory vaguely predicts some mean greater than 0') +
  labs(title = "A: Classic 'nil' NHST: rejecting the null taken as support for the theory") +
  theme_minimal()
  # xkcdaxis(c(-1, 5), c(0, y_max))

weak$x <- rnorm(100, 0.5, 0.5)
mean_x <- mean(weak$x)
...
p2 <-
  ...
  scale_x_continuous(limits = c(-1, 5)) +
  scale_y_continuous(limits = c(0, y_max)) +
  annotate('segment', x = 1, y = arrow_y, xend = 0, yend = arrow_y, arrow = arrw) +
  annotate('text', x = 2.5, y = 25, label = 'Theory predicts mean equals 0, exactly') +
  labs(title = "B: Substantive NHST: rejecting the null rejects the theory") +
  theme_minimal()

weak$x <- rnorm(100, 3, 0.5)
mean_x <- mean(weak$x)
...
p3 <-
  ...
  scale_x_continuous(limits = c(-1, 5)) +
  scale_y_continuous(limits = c(0, y_max)) +
  annotate('segment', x = 1, y = arrow_y, xend = 3, yend = arrow_y, arrow = arrw) +
  annotate('text', x = 1, y = 19, label = 'Theory predicts mean equals 3, exactly') +
  labs(title = "C: Substantive NHST: failing to reject the null fails to reject the theory") +
  theme_minimal()
...
This critical difference in the use of NHST in the natural vs. social sciences was pointed out by Paul Meehl in [1967](https://doi.org/10.1086/288135):
...
We found a huge effect size ($d\sim2$; see @fig-effectsizes, and our p-value was very small $(p = 1.8 \times 10^{-31})$. We therefore rejected the nil and concluded that there was a difference in mean offers. [Publication](http://dx.doi.org/10.1098/rsos.170543)! 

![Effect sizes (absolute values of Cohen's d) from this study (denoted by ‘**’) and a representative sample of previous framing studies. Colours represent game type and bars indicate 95% CI. From Lightner et al. 2017: http://dx.doi.org/10.1098/rsos.170543](rsos170543f13.jpg){#fig-effectsizes}
...
As our substantive $H_0$, we therefore should have proposed this: *There will be no difference between actual offers and self-reported fair offers*. If we had, we would have found that the probability of our data under such an $H_0$ was very small[^5], leading us to reject $H_0$, and therefore to reject our theory. Specifically, our data showed that although many actual offers were exactly, or very close to, self-reported fair offers (dots that fall on, or close to, the diagonal line), as our theory predicted, many other actual offers differed substantially from self-reported fair offers (dots that are far from the diagonal line), contrary to our prediction (@fig-results):

![Self-reported fair offers vs. actual offers in a framed ultimatum game experiment. There were two framed conditions: banker and customer. From Lightner et al. 2017: http://dx.doi.org/10.1098/rsos.170543](F11.large.jpg){#fig-results}
...
[^1]: More precisely, for some test statistic $z$, the p-value is the probability of finding a value of $z$ equal to or more extreme than that observed, under the assumption that $H_0$ is true.

[^2]: Physicists usually demand much smaller p-values than social scientists, e.g., the [$5\sigma$ rule](https://blogs.scientificamerican.com/observations/five-sigmawhats-that/).

[^4]: In a recent example, the OPERA experiment [mistakenly observed](https://en.wikipedia.org/wiki/Faster-than-light_neutrino_anomaly) neutrinos traveling faster than light.

[^5]: To compute confidence intervals, we would need to estimate the precision of our measurements, e.g., how closely do self-reported fair offers correspond to participants' actual beliefs about fair offers? For a detailed example of parameter estimation for a simple physics problem, see [Aguilar et al. 2015](https://doi.org/10.1137/130929230).